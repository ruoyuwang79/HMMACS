...
color[0] = (1, 0, 0)
track_funcs = [test_f.linear(color[i][0] - 0.5, color[i][1] - 0.5, color[i][2] - 0.5) if np.random.rand() < 0.5 else test_f.spiral(np.pi / (50 * color[i][0]), 100 * color[i][1], 5 * color[i][2]) for i in range(n_nodes)]
