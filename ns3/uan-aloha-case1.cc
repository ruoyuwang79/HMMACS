...
Experiment::ResetSim()
{
    ...
    NS_LOG_DEBUG("Average over " << m_avgs << " runs: " << avgThroughput);
}
...
Experiment::UpdatePositions(NodeContainer& nodes) const
{
    ...
    for (; it != nodes.End(); it++)
    {
        ...
        mp->SetPosition(Vector(uv->GetValue(0, m_boundary), uv->GetValue(0, m_boundary), this->m_depth));
    }
}
...
Experiment::Run(UanHelper& uan)
{
...

    {
        ...
        //     Simulator::Schedule(nextEvent, &Experiment::UpdatePositions, this, nc);
        // }
        // Simulator::Schedule(nextEvent, &Experiment::ResetSim, this);
